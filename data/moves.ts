...
export const Moves: {[moveid: string]: MoveData} = {
		...
		condition: {
			...
			onFieldStart(field, source, effect) {
				if (effect?.effectType === 'Ability') {
					this.add('-fieldstart', 'move: Allure Diffusion', '[from] ability: ' + effect, '[of] ' + source);
				} else {
					this.add('-fieldstart', 'move: Allure Diffusion');
				}
			},
			onFieldResidualOrder: 27,
			onFieldResidualSubOrder: 7,
			onFieldEnd() {
				this.add('-fieldend', 'Allure Diffusion');
			},
		},
		secondary: null,
		target: "all",
	...
	attract: {
		...
		condition: {
			...
			onStart(pokemon, source, effect) {
				if (!(pokemon.gender === 'M' && source.gender === 'F') && !(pokemon.gender === 'F' && source.gender === 'M') && !(this.field.isDiffusion('allurediffusion'))) {
	...
	charge: {
		...
		condition: {
			onStart(pokemon, source, effect) {
				if (effect && ['Supercharged'].includes(effect.name)) {
	...
	entrainment: {
		...
		onTryHit(target, source) {
			...
			const additionalBannedSourceAbilities = [
				...
				'flowergift', 'forecast', 'hungerswitch', 'illusion', 'imposter', 'neutralizinggas', 'powerofalchemy', 'trace', 'zenmode',
			];
			if (
	...
	mefirst: {
		...
		onTryHit(target, pokemon) {
			...
			const noMeFirst = [
				'chatter', 'counter', 'covet', 'focuspunch', 'mefirst', 'metalburst', 'mirrorcoat', 'shadowevoboost', 'shadowbath', 'shadowglow', 'shadowflame', 'shadowglaze', 'shadowmoss', 'shadowsprites', 'shadowdestruction', 'shadowdevastation', 'shadowbandit', 'shadowbolt', 'shadowbreak', 'shadowcascade', 'shadowchant', 'shadowcharge', 'shadowchill', 'shadowcinder', 'shadowcombust', 'shadowdance', 'shadowdevour', 'shadowdissolve', 'shadowdoomsday', 'shadowdown', 'shadowend', 'shadowfire', 'shadowfission', 'shadowfog', 'shadowfrost', 'shadowfumes', 'shadowfusion', 'shadowhatred', 'shadowhaunting', 'shadowhold', 'shadowhubris', 'shadowhurl', 'shadowintensify', 'shadowjolt', 'shadowmeld', 'shadowmist', 'shadowmoon', 'shadowpanic', 'shadowphoenix', 'shadowpivot', 'shadowpunish', 'shadowrage', 'shadowrainbow', 'shadowrave', 'shadowreset', 'shadowroulette', 'shadowrush', 'shadowscheme', 'shadowseed', 'shadowshackle', 'shadowshatter', 'shadowshed', 'shadowshuffle', 'shadowsiren', 'shadowslasher', 'shadowsnipe', 'shadowsorcery', 'shadowspell', 'shadowsprint', 'shadowstare', 'shadowstorm', 'shadowsun', 'shadowthreat', 'shadowtitan', 'shadowtrance', 'shadowvampirism', 'shadowvelocity', 'shadowvengeance', 'shadowwall', 'shadowwave', 'shadowwheel', 'shadowwhip', 'shadowwreckage', 'shadowambush', 'shadowanvil', 'shadowbeam', 'shadowblast', 'shadowbluff', 'shadowcentrifuge', 'shadowchew', 'shadowchop', 'shadowdart', 'shadowdesolation', 'shadowdetonation', 'shadowduplicity', 'shadowfirestorm', 'shadowfountain', 'shadowfrenzy', 'shadowgale', 'shadowgluttony', 'shadowhalf', 'shadowinversion', 'shadowlaser', 'shadowmirage', 'shadowmonsoon', 'shadowpress', 'shadowrecruit', 'shadowrequital', 'shadowrobbery', 'shadowsignal', 'shadowsky', 'shadowslander', 'shadowspikes', 'shadowsqueeze', 'shadowstratoslam', 'shadowswap', 'shadowtransform', 'shadowtrip', 'shadowvenom', 'shadowvolley', 'shadowwhiteout', 'shadowwyrm', 'shelltrap', 'struggle', 'thief',
			];
	...
	mysteryroom: {
		...
		condition: {
			...
			// Ability suppression implemented in sim/pokemon.js
			onFieldResidualOrder: 27,
			onFieldResidualSubOrder: 6,
			onFieldEnd() {
				this.add('-fieldend', 'move: Mystery Room');
			},
		},
		secondary: null,
		target: "all",
	...
	naturalshift: {
		...
		onHit(pokemon) {
			...
			if (pokemon.hasType(type) || !pokemon.addType(type)) return;
	...
	passionstorm: {
		...
		onTryMove(attacker, defender, move) {
			...
			this.add('-prepare', attacker, move.name);
			this.boost({spa: 1}, attacker, attacker, move);
			if (this.field.isDiffusion('allurediffusion')) {
				this.attrLastMove('[still]');
	...
	primalinstinct: {
		...
		onTryHit(target, source, move) {
			if (source.species.id === 'unfezantprimal') {
				...
				this.actions.useMove('slipstream', source, target);
			} else if (source.species.id === 'tranquil') {
				this.actions.useMove('slipstream', source, target);
			} else if (source.species.id === 'pidove') {
				this.actions.useMove('slipstream', source, target);
			} else if (source.species.id === 'cherrimprimal') {
				this.actions.useMove('precipiceblades', source, target);
			} else if (source.species.id === 'cherrimsunshine') {
				this.actions.useMove('fieryblossom', source, target);
			} else if (source.species.id === 'cherrim') {
				...
				this.actions.useMove('biddybud', source, target);
			} else if (source.species.id === 'phioneprimal') {
				this.actions.useMove('originpulse', source, target);
			} else if (source.species.id === 'phione') {
				this.actions.useMove('bubbleprison', source, target);
			} else {
				this.actions.useMove('splash', source, target);
			}
			return null;
	...
	roleplay: {
		...
		onTryHit(target, source) {
			...
			const additionalBannedTargetAbilities = [
				// Zen Mode included here for compatability with Gen 5-6
				'flowergift', 'forecast', 'hungerswitch', 'illusion', 'imposter', 'neutralizinggas', 'powerofalchemy', 'trace', 'wonderguard', 'zenmode',
			];
	...
	shadowsqueeze: {
		...
		basePower: 0,
		basePowerCallback(pokemon, target) {
			return Math.floor(Math.floor((100 * (100 * Math.floor(target.hp * 4096 / target.maxhp)) + 2048 - 1) / 4096) / 100) || 1;
		},
		category: "Special",
		name: "Shadow Squeeze",
		pp: 5,
	...
	shadowvampirism: {
		...
		pp: 10,
		priority: 0,
		flags: {contact: 1, protect: 1, heal: 1},
		drain: [1, 2],
		willCrit: true,
		noSketch: true,
		secondary: null,
	...
	shelltrap: {
		...
		condition: {
			...
			onHit(pokemon, source, move) {
				if (!pokemon.isAlly(source) && (move.flags['contact'])) {
	...
	smackdown: {
		...
		condition: {
			...
			onStart(pokemon) {
				let applies = false;
				if (pokemon.hasType('Flying') || pokemon.hasAbility(['levitate', 'inflate'])) applies = true;
	...
	stockpile: {
		...
		condition: {
			...
			onStart(target, source, effect) {
				...
				if (effect.id !== 'supplychain') this.boost({def: 1, spd: 1}, target, target);
	...
	toxicspikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				} else if (pokemon.hasType('Steel') || pokemon.hasItem('heavydutyboots') || pokemon.hasAbility('wonderguard')) {
					return;
