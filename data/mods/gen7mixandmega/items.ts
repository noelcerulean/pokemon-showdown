export const Items: {[k: string]: ModdedItemData} = {
	...
	emeraldorb: {
		...
		onPrimal(pokemon) {
			// @ts-ignore
			const species: Species = this.actions.getMixedSpecies(pokemon.m.originalSpecies, 'Unfezant-Primal');
			if (pokemon.m.originalSpecies === 'Unfezant') {
				pokemon.formeChange(species, this.effect, true);
			} else {
				pokemon.formeChange(species, this.effect, true);
				pokemon.baseSpecies = species;
				this.add('-start', pokemon, 'Emerald Orb', '[silent]');
				...
				if (pokemon.illusion) {
					...
					if (types.length > 1 || types[types.length - 1] !== 'Flying') {
						this.add('-start', pokemon, 'typechange', (types[0] !== 'Flying' ? types[0] + '/' : '') + 'Flying', '[silent]');
					}
	...
	ceriseorb: {
		...
		onPrimal(pokemon) {
			// @ts-ignore
			const species: Species = this.actions.getMixedSpecies(pokemon.m.originalSpecies, 'Cherrim-Primal');
			if (pokemon.m.originalSpecies === 'Cherrim') {
				pokemon.formeChange(species, this.effect, true);
			} else {
				pokemon.formeChange(species, this.effect, true);
				pokemon.baseSpecies = species;
				this.add('-start', pokemon, 'Cerise Orb', '[silent]');
				const apparentSpecies = pokemon.illusion ? pokemon.illusion.species.name : pokemon.m.originalSpecies;
				const oSpecies = this.dex.species.get(apparentSpecies);
				if (pokemon.illusion) {
					const types = oSpecies.types;
