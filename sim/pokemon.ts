	...
	*/
	...
	ignoringAbility() {
		...
		return !!(
			...
			((this.volatiles['gastroacid'] || (neutralizinggas && this.ability !== ('neutralizinggas' as ID)) && !this.getAbility().isPermanent || this.battle.field.pseudoWeather['mysteryroom']) &&
	...
	formeChange(
		...
		if (isPermanent && !['disguise', 'iceface', 'mimictree'].includes(source.id)) {
			if (this.illusion) {
	...
	isGrounded(negateImmunity = false) {
		...
		if (item === 'magneticstone' && this.species.num === 476) return false;
	...
	isProtected() {
		return !!(
			...
			this.volatiles['kingsshield'] || this.volatiles['spikyshield'] || this.volatiles['regroup2'] ||
