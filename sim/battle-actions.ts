	...
	hitStepBreakProtect(targets: Pokemon[], pokemon: Pokemon, move: ActiveMove) {
		if (move.breaksProtect) {
			for (const target of targets) {
				let broke = false;
				for (const effectid of ['banefulbunker', 'kingsshield', 'obstruct', 'protect', 'regroup', 'spikyshield']) {
		...
		for (const [i, target] of targets.entries()) {
			...
			if (target) {
				...
				if (moveData.heal && !target.fainted) {
					...
					const d = this.battle.heal(amount, target, source, move);
					if (!d && d !== 0) {
						this.battle.add('-fail', source);
						this.battle.attrLastMove('[still]');
				...
				if (moveData.diffusion) {
					hitResult = this.battle.field.setDiffusion(moveData.diffusion, source, move);
					didSomething = this.combineResults(didSomething, hitResult);
				}
