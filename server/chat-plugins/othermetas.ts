...
export const commands: Chat.ChatCommands = {
	...
	mixandmega(target, room, user) {
		...
		if (!stone || (dex.gen >= 8 && ['redorb', 'blueorb', 'tealorb', 'ceriseorb', 'emeraldorb', 'craftsmanorb'].includes(stone.id))) {
		...
		} else if (stone.id === 'emeraldorb') {
			megaSpecies = dex.species.get("Unfezant-Primal");
			baseSpecies = dex.species.get("Unfezant");
		} else if (stone.id === 'craftsmanorb') {
			megaSpecies = dex.species.get("Regigigas-Primal");
			baseSpecies = dex.species.get("Regigigas");
		}
	...
	stone(target) {
		...
		for (const aStone of toDisplay) {
			...
			if (dex.gen >= 8 && ['redorb', 'blueorb', 'tealorb', 'ceriseorb', 'emeraldorb', 'craftsmanorb'].includes(aStone.id)) {
				throw new Chat.ErrorMessage("The Orbs do not exist in Gen 8 and later.");
			}
			if (aStone.id === 'redorb') { // Orbs do not have 'Item.megaStone' or 'Item.megaEvolves' properties.
				...
				megaSpecies = dex.species.get("Cherrim-Primal");
				baseSpecies = dex.species.get("Cherrim");
			} else if (aStone.id === 'emeraldorb') {
				megaSpecies = dex.species.get("Unfezant-Primal");
				baseSpecies = dex.species.get("Unfezant");
			} else if (aStone.id === 'craftsmanorb') {
				megaSpecies = dex.species.get("Regigigas-Primal");
				baseSpecies = dex.species.get("Regigigas");
			}
			const deltas: StoneDeltas = {
			...
			let tier;
			if (['redorb', 'blueorb', 'tealorb', 'ceriseorb', 'emeraldorb', 'craftsmanorb'].includes(aStone.id)) {
				tier = "Orb";
